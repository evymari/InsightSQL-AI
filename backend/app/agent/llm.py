    ...
    def _setup_client(self):
        if self.provider == "openai":
            ...
            if not settings.openai_azure_api_key or not settings.openai_azure_endpoint:
                raise ValueError("OPENAI_AZURE_API_KEY and OPENAI_AZURE_ENDPOINT are required for Azure OpenAI")
            # Para Azure OpenAI, necesitamos extraer el endpoint base
            base_url = settings.openai_azure_endpoint.rstrip('/') + "/openai/deployments/" + settings.openai_model
            self.client = AsyncOpenAI(
                api_key=settings.openai_azure_api_key,
                base_url=base_url,
                api_version=settings.openai_azure_version
            )
            self.model = settings.openai_model
